...
export const useSystemCalls = () => {
    ...
    const spawn = async () => {
        ...
        state.applyOptimisticUpdate(transactionId, (draft) => {
            if (draft.entities[entityId]?.models?.dojo_starter?.Capy) {
                draft.entities[entityId].models.dojo_starter.Capy.hearts =
                    HEARTS;
            }
        });
        ...
        try {
            ...
            await state.waitForEntityChange(entityId, (entity) => {
                return (
                    entity?.models?.dojo_starter?.Capy?.hearts ===
                    HEARTS
                );
            });
        } catch (error) {
    ...
    const give_heart = async () => {
        ...
        state.applyOptimisticUpdate(transactionId, (draft) => {
            if (draft.entities[entityId]?.models?.dojo_starter?.Capy) {
                HEARTS = draft.entities[entityId].models.dojo_starter.Capy.hearts;
                if (HEARTS < 3) draft.entities[entityId].models.dojo_starter.Capy.hearts += 1;
            }
        });

        try {
            // Execute the spawn action from the client
            await client.actions.giveHeart(account!, Date.now());

            // Wait for the entity to be updated with the new state
            await state.waitForEntityChange(entityId, (entity) => {
                return (
                    entity?.models?.dojo_starter?.Capy?.hearts === HEARTS+1
                );
            });
        } catch (error) {
            // Revert the optimistic update if an error occurs
            state.revertOptimisticUpdate(transactionId);
            console.error("Error executing give_hearts:", error);
            throw error;
        } finally {
            // Confirm the transaction if successful
            state.confirmTransaction(transactionId);
